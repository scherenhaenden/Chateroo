...
export class ChatService {
  ...
  sendMessage(payload: SendMessageDto): Promise<ChatResponse> {
    ...
    const chatPayload: ChatPayload = {
      ...
      attachments:
        payload.attachments ||
        payload.messages?.[payload.messages.length - 1]?.attachments,
    };
  ...
  async getOpenRouterModels(apiKey?: string): Promise<OpenRouterModel[]> {
    ...
    if (!apiKey) {
      try {
        ...
        console.log(
          'No API key provided for OpenRouter models, returning empty list',
        );
        return [];
      }
    }
  ...
  > {
    ...
    for (const providerId of providers) {
      try {
        ...
        console.error(
          `Error fetching models for provider ${providerId}:`,
          error,
        );
        result[providerId] = [];
      }
  ...
  > {
    ...
    switch (providerId) {
      case 'openrouter':
        ...
        const openRouterModels = await openRouterEngine.listModels(
          apiKey || '',
        );
    ...
    return [
      {
        id: 'local-model',
        name: 'Local Model',
        description: 'Currently loaded local model',
        context_length: 4096,
      },
    ];
  }
